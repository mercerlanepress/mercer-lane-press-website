...
for(const name of readdirSync(bookPagesDir)){
  if(!name.endsWith('.astro') || ['[slug].astro','index.astro'].includes(name)) continue;
  const source=readFileSync(join(bookPagesDir,name),'utf8');
  if(/featuredBook\s+as\s+book/.test(source)) throw new Error(`Book page must select its own record instead of featuredBook: ${name}`);
}
...
for(const file of html){
  ...
  if(metadata.has(key)) throw new Error(`Duplicate page metadata: ${title}`);
  metadata.add(key);
  for(const href of source.matchAll(/href="(\/[^"#?]*)/g)){
    ...
    if(path.startsWith('/images/')||path.includes('.')){
      ...
      if(!existsSync(target)) throw new Error(`Broken internal link ${path} in ${file}`);
    }
  }
  for(const image of source.matchAll(/<img[^>]+src="(\/[^"?]+)"[^>]*>/g)){
    if(!existsSync(join(dist,image[1]))) throw new Error(`Missing image ${image[1]} in ${file}`);
    if(!/\salt="[^"]+"/.test(image[0])) throw new Error(`Image without useful alt text in ${file}`);
  }
}
...
for(const file of html){
  const source=readFileSync(file,'utf8');
  for(const url of source.matchAll(/<link rel="canonical" href="([^"]+)"/g)) if(!url[1].startsWith('https://mercerlanepress.com/')) throw new Error(`Bad canonical URL in ${file}`);
}

console.log(`Site checks passed for ${html.length} HTML pages: build, whitespace, links, images/alt text, purchase URLs, book-page source guards, sitemap, robots, canonicals, and unique metadata.`);
